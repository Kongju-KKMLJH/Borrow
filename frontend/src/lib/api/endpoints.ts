...
export const adminApi = {
  /** 7.1.1 전체 회원 목록 (ADMIN) — 관리용 전체 목록 */
  ...
  /** 7.1.2 회원 수정 (ADMIN) — password 를 비우면 기존 값 유지. 대상이 ADMIN 이면 거절된다 */
  updateUser: (userId: number, body: AdminUserRequest) =>
    apiClient.put<AdminUserResponse>(`/api/admin/users/${userId}`, body),

  /**
   * 7.1.2 회원 삭제 (ADMIN) — 연쇄 하드 삭제.
   * 이 회원의 인증 신청·참여 신청·개설 프로그램·등록 공간까지 함께 지워진다. 대상이 ADMIN 이면 거절된다.
   */
  ...
  /** 7.2.1 전체 프로그램 목록 (ADMIN) — 상태를 가리지 않는다 */
  ...
  /** 7.2.2 프로그램 수정 (ADMIN) — 개최 요청은 지우고 다시 만든다 */
