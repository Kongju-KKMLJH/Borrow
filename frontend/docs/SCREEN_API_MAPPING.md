...
기능명세 7.1.2 · 7.2.2 · 7.3.2의 **임시(mock) 데이터 생성·수정·삭제**는 별도 이슈로 진행한다.
목록 응답에는 `mock` 필드가 이미 내려오고 화면도 배지로 표시하지만, 생성·수정 폼과
`POST`/`PUT`/`DELETE /api/admin/{users,activities,spaces}`는 아직 없다.
